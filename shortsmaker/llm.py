...
__all__ = ["LLMUnavailable", "available", "status", "complete", "set_provider", "logout",
           ...
           "gen_mbti_hooks", "gen_one_mbti_hook", "gen_mbti_captions", "gen_hashtags",
           "shorts_meta"]
...
def gen_fill(title: str, scenes: list, review_of: dict | None = None,
    ...
    if want_hook:
        fmt += ("후크1: (검정으로 나갈 1줄, 호기심·도발 질문형, 16자 이내)\n"
                ...
                "해시태그: 콘텐츠 키워드 중심 12~16개(넉넉하게). 한 줄, 공백 구분, 각 #로 시작\n")
    if want_caps:
...
def gen_hashtags(title: str, scenes: list) -> str:
    """장 내용으로 유튜브 쇼츠 해시태그를 넉넉히(12~16개) 생성. '#a #b ...' 한 줄 반환."""
    briefs = "\n".join(f"씬{s.get('scene_index')}: {(s.get('narration') or s.get('subtitle') or '')[:160]}"
                       for s in scenes)
    raw = complete(_prompt("hashtags", title=title, briefs=briefs), max_tokens=200)
    for line in (raw or "").splitlines():
